...
# ////////////////////////// club//////////////
class viewinstructions(View):
    def get(self,request):
        obj = event.objects.all()
        return render(request,'clubs/viewinstructions.html',{'val':obj})
...
class clubnotification(View):
    def get(self,request):
        obj = NotificationTable.objects.all()
        return render(request,'clubs/clubnotification.html',{'val':obj})
class complaints(View):
    def get(self,request):
        c=request.session.get('user_id')
        d=LoginTable.objects.get(id=c)
        return render(request,'clubs/complaints.html',{'c':d})
    def post(self, request):
...
class addstudent(View):
    def get(self,request):
      ...
      d=LoginTable.objects.get(id=c)
      print(d) 
      st=addstudents.objects.filter(CLUB_ID_id=d.id)
      print(st)
      return render(request,'clubs/addstudents.html',{'c':d,'st':st})
    def post(self, request):
        print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
        print(request.session.get('user_id'))
        c=addstudentsform(request.POST)
        if c.is_valid():
            c.save(commit=False)
            d=LoginTable.objects.create(username=request.POST['username'],password=request.POST['password'],usertype='student')
            c.STUDENT_ID=d
            c.save()
            return redirect('addstudents')

class remove_student(View):
    def get(self,request,id):
        obj=addstudents.objects.get(id=id)
        obj.delete()
        return HttpResponse('''<script>alert("DELETED");window.location="/addstudents"</script>''')      
...
class feedback(View):
    def get(self,request):
        c=request.session.get('user_id')
        d=LoginTable.objects.get(id=c)
        return render(request,'clubs/feedback.html',{'c':d})
    def post(self, request):
        print(request.session.get('user_id'))
